...
def base_experiment_config(dataset):
    ...
    DESCRIPTION = """ Re-Impl. of Zhangs's Seq2Seq
    """
...
class _Seq2SeqNetPaddingSame(nn.Module):
    def __init__(self, SEQ_LENGTH):
        super(_Seq2SeqNetPaddingSame, self).__init__()
        ...
        self.conv1 = nn.Sequential(
            ...
            nn.Conv1d(40, 50, 5, stride=1, bias=True),
            nn.ReLU(True),
            nn.ConstantPad1d((2, 2), 0),
            nn.Conv1d(50, 50, 5, stride=1, bias=True),
            nn.ReLU(True),
        )
        ...
        self.dense2 = nn.Linear(1024, 599)
...
def load_disaggregator(EVALUATION_DATA_PATH, MODEL_PATH, config=None, USE_CUDA=True):
    ...
    target_processing_steps = [Add(-INPUT_MEAN), DivideBy(INPUT_STD), Transpose((0, 2, 1))]
