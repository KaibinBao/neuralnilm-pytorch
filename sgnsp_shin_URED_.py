...
def base_experiment_config(dataset):
    ...
    DESCRIPTION = """ Re-Impl. of Shin's Subtask Gated Network with standby power (SGN-SP)
    """
...
def disag_seq2seq_sgn(model, mains, target, input_processing, target_processing, max_target_power, n_seq_per_batch, seq_length, target_seq_length, building_i, stride, USE_CUDA, return_sequences):
...
class _SGNShinSubNetPaddingSame(nn.Module):
    ...
    def forward(self, input):
        ...
        x = self.act1(x)
        x = self.dense2(x)
        x = x.view(-1, 1, self.target_seq_length)
        return x


class _SGNSPShinNet(nn.Module):
    def __init__(self, SEQ_LENGTH, TARGET_SEQ_LENGTH):
        super(_SGNSPShinNet, self).__init__()

        self.seq_length = SEQ_LENGTH
        self.regression = _SGNShinSubNetPaddingSame(SEQ_LENGTH, TARGET_SEQ_LENGTH)
        self.gate = _SGNShinSubNetPaddingSame(SEQ_LENGTH, TARGET_SEQ_LENGTH)
        self.sigmoid = nn.Sigmoid()
        self.standby_power = nn.parameter.Parameter(torch.Tensor(1))
        self.standby_power.data.zero_()
    ...
    def forward(self, input):
        x = self.regression(input)
        g = self.gate(input)
        sg = self.sigmoid(g)
        x = (sg*x)+((1-sg)*self.standby_power)
        return x, g
