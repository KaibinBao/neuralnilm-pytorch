...
def base_experiment_config(dataset):
    ...
    DESCRIPTION = """ Re-Impl. of Chen's Seq2Seq with GLUs
    """
...
class _ResBlock(nn.Module):
    def __init__(self, n_inoutput, n_hidden):
        super(_ResBlock, self).__init__()
        self.residual = nn.Sequential(
            nn.Linear(n_inoutput, n_hidden),
            nn.ReLU(True),
            nn.Linear(n_hidden, n_inoutput)
        )
...
class _Seq2SeqChen(nn.Module):
    def __init__(self, SEQ_LENGTH, TARGET_SEQ_LENGTH):
        super(_Seq2SeqChen, self).__init__()
        
        self.seq_length = SEQ_LENGTH
        self.target_seq_length = TARGET_SEQ_LENGTH
        self.nc  = 1 # number of channels
        self.n_dense_input = (SEQ_LENGTH//8)*100

        self.glunet = nn.Sequential(
            _GLUBlock(1, 100),
            _GLUBlock(100, 100),
            _GLUBlock(100, 100),
        )

        self.fc1 = nn.Linear(self.n_dense_input,100)
    ...
    def forward(self, input):
        x = self.glunet(input)
        x = self.fc1(x.view(-1, self.n_dense_input))
        x = self.act1(x)
        x = self.resnet(x)
        x = self.fc2(x)
...
def load_disaggregator(EVALUATION_DATA_PATH, MODEL_PATH, config=None, USE_CUDA=True):
    ...
    offset = (SEQ_LENGTH - TARGET_SEQ_LENGTH) // 2
    input_processing_steps = [DivideBy(INPUT_STD), Transpose((0, 2, 1))]
    target_processing_steps = [SubSequence(offset,-offset), DivideBy(TARGET_STD), Transpose((0, 2, 1))]
...
def run_experiment(dataset,
    ...
    input_processing_steps += [DivideBy(INPUT_STD), Transpose((0, 2, 1))]
    target_processing_steps = [groundtruth_processing, DivideBy(TARGET_STD), Transpose((0, 2, 1))]
