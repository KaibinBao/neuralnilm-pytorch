...
def base_experiment_config(dataset):
    ...
    DESCRIPTION = """ Re-Impl. of Zhangs's Seq2Point
    """
...
class _Seq2PointNetPaddingSame(nn.Module):
    def __init__(self, SEQ_LENGTH):
        super(_Seq2PointNetPaddingSame, self).__init__()
        ...
        self.conv1 = nn.Sequential(
            ...
            nn.Conv1d(self.nc, 30, 10, stride=1),
            nn.ReLU(True),
...
def run_experiment(dataset,
    ...
    target = torch.FloatTensor(NUM_SEQ_PER_BATCH, 1, 1)
        ...
        for fold in validation_batches:
            ...
            for batch in validation_batches[fold]:
                ...
                if USE_CUDA:
                    ...
                    target_signal = target_signal.cuda()
                with torch.no_grad():
                    output = net(aggregated_signal)
                    val_loss = criterion(output, target_signal)
