...
from neuralnilm.data.processing import DivideBy, IndependentlyCenter, Transpose, Add, SubSequence, DownSample
...
def base_experiment_config(dataset):
    ...
    TARGET_SEQ_LENGTH = 2053
    SEQ_LENGTH = TARGET_SEQ_LENGTH + 2052

    if TARGET_APPLIANCE == 'dish washer':
        TARGET_ON_MEAN = 1059.8963789506392
    elif TARGET_APPLIANCE == 'fridge':
        SOURCE_TYPES = ["BalancedBuildingRealAggregateSource"]
        TARGET_ON_MEAN = 109.11351108551025
    elif TARGET_APPLIANCE == 'kettle':
        TARGET_ON_MEAN = 2127.9581037248886
    elif TARGET_APPLIANCE == 'microwave':
        TARGET_ON_MEAN = 738.0303453717913
    elif TARGET_APPLIANCE == 'television':
        TARGET_ON_MEAN = 85.93876961299351
    elif TARGET_APPLIANCE == 'tumble dryer':
        TARGET_ON_MEAN = 1211.016840616862
    elif TARGET_APPLIANCE == 'washing machine':
        TARGET_ON_MEAN = 576.8361129760742

    INPUT_MEAN = 500.0
    INPUT_STD = 700.0

    MAX_TARGET_POWER = 4200

    TRAINING_SEED = 42
    VERBOSE_TRAINING = True
    DOWNSAMPLE_FACTOR = 0

    DESCRIPTION = """ Re-Impl. of Brewitts's Fully Convolutional Network
    """
    ...
    TEST_DISAGGREGATE_EVERY_N_EPOCHS = None
...
def get_validation_batches(data_pipeline, VALIDATION_SOURCE_TYPES, NUM_BATCHES_FOR_VALIDATION):
    ...
    shortname_folds = {
        #"train": "training",
        #"unseen_appliances": "unseen_appliances",
...
class _FCNBrewitt(nn.Module):
    def __init__(self):
        super(_FCNBrewitt, self).__init__()
        
        self.nc  = 1 # number of channels
        self.nf = 128
        self.n_stacked_convs = 9

        self.stem = nn.Conv1d(self.nc, self.nf, 9, stride=1)
        
        convs = [nn.ReLU(True)]
        for i in range(self.n_stacked_convs):
            d = 2**(i+1)
            c = nn.Conv1d(self.nf, self.nf, 3, dilation=d)
            convs.append(c)
        ...
        self.head = nn.Sequential(
            nn.Conv1d(self.nf, self.nf, 1),
            nn.ReLU(True),
            nn.Conv1d(self.nf, 1, 1)
        )
...
def load_disaggregator(EVALUATION_DATA_PATH, MODEL_PATH, config=None, USE_CUDA=True):
    ...
    TARGET_ON_MEAN =        config['TARGET_ON_MEAN']
    ...
    target_processing_steps = [SubSequence(offset,offset+TARGET_SEQ_LENGTH), DivideBy(TARGET_ON_MEAN), Transpose((0, 2, 1))]
...
def run_experiment(dataset,
    ...
    sources, validation_sources = get_sources(
        ...
        validation_stride=TARGET_SEQ_LENGTH )

    offset = (SEQ_LENGTH - TARGET_SEQ_LENGTH) // 2
    input_processing_steps = [DivideBy(INPUT_STD), IndependentlyCenter(), Transpose((0, 2, 1))]
    groundtruth_processing = SubSequence(offset,offset+TARGET_SEQ_LENGTH)
    target_processing_steps = [groundtruth_processing, DivideBy(TARGET_ON_MEAN), Transpose((0, 2, 1))]
    ...
    # Trace the Network
    #traced_net = torch.jit.trace(net, input)
    #net = traced_net
